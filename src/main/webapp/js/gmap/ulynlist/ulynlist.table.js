...
//       var th_checkbox = $(".th_checkbox", $(obj));
    ...
    function addTemplateHelper() {
        ...
        template.helper('$outputContent', function (item, column,currentObj,rowIdx) {
            return outputContent(item, column,currentObj,rowIdx);
        });
        //输出内容，用于title属性，主要是过滤去除html标签以及替换双引号为 &quot;
        template.helper('$outputContentForTitle', function (item, column,currentObj,rowIdx) {
            if(item==null){
                return null;
            }
        ...
        template.helper('$doFunc', function (fn) {
            if(fn!= null){
                var args = [];
    ...
    function standardParam(opts) {
        ...
        //show为false的不渲染
        arrayFilter(opts.tableColumn.columns,function(item,i){
