...
(function($) {
    ...
    function createUlynlist(opts,obj){
        opts.beforeLoad();
        ...
        if(opts.customData!=null){
            createUlynlistForCustomData(obj,opts);
        }else{
            //使用服务端请求数据
            opts.customAjax(opts.requestData,function(data){
                createUlynlistForAjaxData(obj,opts,data);
            },opts);
        }
    ...
    function createUlynlistForCustomData(obj,opts){
        //使用自定义传入的数据显示
        if(opts.usePageBar){
            //有使用pageBar，则要进行分页
...
//                opts.startRowNum = (opts.currentPage-1)*opts.linesPerPage;
    ...
    function createUlynlistForAjaxData(obj,opts,data){
        if(data.status){
...
//                            opts.startRowNum = (opts.currentPage-1)*opts.linesPerPage;
    ...
    $.fn.ulynlist.combine = function(arr){
        ...
        mainOpts = $.extend(true,{}, $.fn.ulynlist.defaults, mainOpts);
        var pageBar = $("#"+mainOpts.pageBarId);
        var mainCustomAjax = mainOpts.customAjax;
        var dataTemp = {"data":{"currentPage":1,linesPerPage:1,"list":[],"totalNum":0},"msg":"","status":true};
        mainOpts.customAjax = function(requestData,callBackFunc,opts){
            ...
            mainCustomAjax(requestData,callback,opts);
        }
        for(var i= 1,l=arr.length;i<l;i++){
            var item = arr[i];
            var obj = item['obj'];
            ...
            opts.customAjax = function(requestData,callBackFunc,opts){
                callBackFunc(dataTemp);
            }
            obj.ulynlist(opts);
        }
        //主表做取数据的
    ...
    $.fn.ulynlist.getCurrentParams = function(ulynlistObj){
        return $(ulynlistObj)[0].getCurrentParams();
    }
    /**
     * 导出查询的数据
     * @param ulynlistObj
