...
;(function (factory) {
    if (typeof define === "function" && define.amd) {
        // AMD模式
        define("ulynlist.pagebar",[ "jquery","artTemplate" ], factory);
    } else {
        // 全局模式
...
//        $(obj).data(data_key_pagebar_opts,opts);
        ...
        if (opts.afterPaginationRender) {
            //渲染后回调的方法
            opts.afterPaginationRender($(obj),opts);
        }
    }
...
//        opts.totalPage = Number(opts.totalPage);
        ...
        opts.totalPage = Math.ceil(opts.totalNum/opts.linesPerPage);
...
//        totalPage:0,   //总页数  废弃：此参数由其他参数可以算出
        ...
        extra:{}//额外一些数据定义，一般没用，除了自定义模版参数呈现可能会有用，
                // 比如：// flat模版  linesPerPageEditable:true,  //每页几行的输入框是否可以编辑
    };
    ...
    (function(tpl){
        //根据要显示的页面按钮的数组
        tpl.helper('$outPageBtn', function (opts) {
            var sb = "";
            var start = 1;
